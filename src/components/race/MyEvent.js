      ...
      <div className="container">
                      ...
                      <Col lg="8" md="6">
                        <Media >
                          <Link to={event.event.category.name === 'Run' ? this.getRunLink(event.id) : this.getLink(event.id, event.status)}>
                            <Image className="mr-3" height="64" src={event.event.cover_thumb === '' ? `${IMAGE_URL}${event.event.cover}` : `${IMAGE_URL}${event.event.cover_thumb}`} rounded /></Link>
                          <Media.Body>
        ...
        <Row >
          {/* {this.state.events.map((event, i) => {
            ...
            return (
              <div style={{ cursor: 'pointer' }} className="col-sm-6 col-md-3" id="p1" key={i}>
                <div className="card shadow border-0" onClick={this.onClick.bind(this, event)}>

                  <Image src={`${IMAGE_URL}${event.event.cover}`} thumbnail />
                  ...
                  <div className="card-body p-2">
                    ...
                    <small className="event-date-label">
                      <cite title="Source Title">
                        <span className="event-date-start">{utils.convertDateApiToMMMM_DD_YYYY(event.start_event)} </span>
                        <span className="event-date-end">{utils.convertDateApiToMMMM_DD_YYYY(event.start_event)}</span>
                      </cite>
                    </small>
                    <h5 className="card-title">{event.name}</h5>

                    <p className="card-text text-muted ellipsis"><img
                      src={iconmarker}
                      height="18"
                      className="mr-1"
                      alt="Runex logo"
                    ...
                    <div className="list-reg-images">
                      <label style={{ color: this.statusColor(event.status) }}>{event.status + ' :: ' + this.statusTH(event.status)}</label>
